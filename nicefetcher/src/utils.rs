...
use std::panic;
use crate::indexer::bitcoin_client::hash_string;
use crate::indexer::bitcoin_client::hash_string_and_number;
...
#[pyfunction]
fn pack_address(address: &str) -> PyResult<Vec<u8>> {
    Ok(hash_string(address))
}


#[pyfunction]
fn pack_utxo(txid: Vec<u8>, vout: u64) -> PyResult<Vec<u8>> {
    Ok(hash_string_and_number(&txid, vout))
}
...
pub fn register_utils_module(parent_module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(pack_utxo, &m)?)?;
